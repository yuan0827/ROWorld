        ...
        const groupsHTML = visibleGroups.map(group => {
            ...
            const list = groupMembers.map(m => {
                ...
                if (isGVG) {
                    ...
                    // [GVG Light Fix v7.2] Use inline Tailwind, ensure pointer events
                    // Leave Light (Yellow indicator)
                    const leaveLight = `<div class="w-3 h-3 rounded-full border border-white shadow-sm cursor-pointer transition hover:scale-125 ${m.status === 'leave' ? 'bg-yellow-400' : 'bg-slate-200'}" title="請假"></div>`;
                    // Status Light (Red/Green Toggle)
                    const statusColor = m.status === 'ready' ? 'bg-green-500' : 'bg-red-500';
                    const statusLight = `<div class="w-3 h-3 rounded-full border border-white shadow-sm cursor-pointer transition hover:scale-125 ${statusColor}" title="狀態: ${m.status}" onclick="event.stopPropagation(); app.toggleGvgStatus('${group.id}', '${m.id}', 'ready_toggle')"></div>`;
            ...
            return `<div class="${cardClass}"><div class="${headerClass} flex justify-between items-center"><div><div class="flex items-center mb-1">${dateBadge}${subjectBadge}</div><h3 class="text-xl font-bold text-slate-800">${group.name}</h3><p class="text-xs mt-1 italic text-slate-500">${group.note||''}</p></div><div class="flex items-center">${copyBtn}${editBtn}</div></div><div class="flex-grow overflow-y-auto max-h-80 custom-scrollbar">${list.length ? list : '<p class="text-sm text-slate-400 text-center py-4">無成員 (或被篩選隱藏)</p>'}</div>${footer}</div>`;
        }).join('');
